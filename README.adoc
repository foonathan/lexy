...
Instead of manually writing a recursive descent parser, you can describe your parser in this DSL and the library will generate it for you.
The generated parser behaves just like you've specified in the grammar
-- it tries all alternatives in the order you said it should, it does not do backtracking unless you allow it, and it will fail with a custom error message when you instruct it to.
Values and lexemes produced by the grammar are directly converted to your custom data structure without any intermediate copies or heap allocations,
and parse errors are handled by a callback you have specified.
This gives you all the flexibility of a hand-written parser without any loss of control and manual labor.
