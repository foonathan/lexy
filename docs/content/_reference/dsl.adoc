...
no_trailing_sep(branch)
no_trailing_sep(branch).error<Tag>
----
...
Use `no_trailing_sep(branch)` to indicate that `branch` has to be consumed between two items.
If it would match after the last item, an error is raised.
It has the tag `lexy::unexpected_trailing_separator` unless a different one was specified using `.error`.
...
Values::
  ...
  The captured lexeme spans over multiple tokens; there isn't a separate one for each occurrence.
Errors::
