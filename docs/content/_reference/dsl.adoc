...
`lexy::plus_sign` and `lexy::minus_sign` are empty tag types that are implicitly convertible to `+1`/`-1`.
...
Values:::
  If a `+` sign was consumed, the value is `lexy::plus_sign`.
  If a `-` sign was consumed, the value is `lexy::minus_sign`.
  If no sign was consumed, the value is `lexy::plus_sign`.
Errors::
  n/a (they don't fail)
