...
The member function `.or_default()` provides a fallback that can produce a default constructed value of any type via implicit conversion.
...
They initialize their result of type `T` from `init` and then fold with every invocation of the sink callback:
...
It returns the number of invocations of the sink callback as a `std::size_t`.
