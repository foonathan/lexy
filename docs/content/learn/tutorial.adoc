...
int main()
{
    ...
    auto result = lexy::parse<grammar::config>(file.buffer(), report_error_callback); <4>
    if (!result) <5>
    { … }
...
<4> Then we can parse our entry production using `lexy::parse`.
    We give it the buffer of our file and a callback to invoke on errors (not shown here).
...
<2> If we don't have a dot, this isn't an issue.
