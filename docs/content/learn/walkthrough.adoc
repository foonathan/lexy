...
Following the three steps outlined in the {{< ref "learn/warmup" >}}[warmup], our program has the following structure:
...
As discussed in the {{< ref "learn/branching" >}}[branching tutorial], we need to use {{% branch-rule %}}s to let lexy know how to make a decision.
...
As discussed in the {{< ref "learn/branching" >}}[tutorial about branching], they allow parsing things without requiring a branch condition.
...
Likewise, we can use {{% docref "dsl::peek_not" %}}, which fails if a rule would match, if we were to specify a build string in our version:
