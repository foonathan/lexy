...
`lexy::make_buffer` is a function object that constructs a `lexy::buffer` of the specified encoding from raw memory.
...
constexpr /* see below */ name() const noexcept; // <2>

constexpr TokenKind get() const noexcept; // <3>
----
<1> Returns `true` if the token kind is not unknown, `false` otherwise.
<2> Returns the name of the token kind, the return type is some unspecified type like `std::string_view`.
...
* Otherwise, if ADL finds an overload `string-like token_kind_name(TokenKind kind)`,
  where `string-like` is either `const char*` or a class with `.data()` and `.size()`,
...
Either by calling `.kind()` on the token rule and giving it a value there,
