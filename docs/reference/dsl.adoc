...
.`lexy/dsl/peek.hpp`
----
peek_not(pattern) : Pattern
----

The `peek_not` pattern matches a pattern without consuming anything but flips its success and failure; only succeeding if the pattern did not match.
This can be used for simple lookahead in a branch condition.
It is whitespace aware; leading whitespace will be consumed.
...
Matches::
  Matches the pattern.
  If it succeeds, consumes nothing and fails.
