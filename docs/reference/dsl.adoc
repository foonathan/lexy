...
----
context_flag<Id>.check() : Branch
----

The `.check()` rule does not interact with the input at all.
When it is used as a branch condition that branch is only taken if the value of the flag with the given `Id` is `true`.
When it is parsed normally it has no effect.
...
----
context_counter<Id>.check_eq<Value>() : Branch
context_counter<Id>.check_lt<Value>() : Branch
context_counter<Id>.check_gt<Value>() : Branch
----

The `.check_*()` rules do not interact with the input at all.
When it is used as a branch condition that branch is only taken if the value of the counter with the given `Id` is equal to/less than/greater than `Value`.
When it is parsed normally it has no effect.
