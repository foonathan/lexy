...
* *Behavior change*: `dsl::try_()` behaves differently when recovering from a branch.
  Previously, when `dsl::try_(condition >> then)` is used and `condition` fails, it will have consumed part of `condition` and starts recovery there.
  Now, it backtracks `condition`.
  If `then` fails the behavior is unchanged and `condition` and the part of `then` already consumed is not backtracked.
