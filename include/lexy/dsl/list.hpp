...
struct _list_done
{
    template <typename Context, typename Input, typename Sink, typename... Args>
    LEXY_DSL_FUNC auto parse(Context& context, Input& input, Sink& sink, Args&&... args) ->
...
{
    template <typename Context, typename Input, typename Sink, typename... Args>
    LEXY_DSL_FUNC auto parse(Context& context, Input& input, Sink& sink, ParentArgs&&... pargs,
                             Args&&... args) -> typename Context::result_type
    {
        sink(LEXY_FWD(args)...); // Forward item args to the sink.
        return NextParser::parse(context, input, sink, LEXY_FWD(pargs)...); // Continue.
    }
};
...
struct _list_loop_parser<Item, void, NextParser> // no separator
{
        ...
        typename Context::result_type
    {
        ...
        if (Item::condition_matcher::match(input))
        {
            ...
            return Item::template then_parser<continuation>::parse(context, input, sink,
                                                                   LEXY_FWD(args)...);
        }
        else
        {
            return _list_done<NextParser>::parse(context, input, sink, LEXY_FWD(args)...);
        }
    }
};
template <typename Item, typename Sep, typename NextParser>
struct _list_loop_parser<Item, _sep<Sep>, NextParser> // normal separator
{
        ...
        typename Context::result_type
    {
        ...
        if (Sep::matcher::match(input))
        {
            // We parse item, then the done continuation, and then we jump back here.
            using continuation = _list_item_done<_list_loop_parser, Args...>;
            return Item::template parser<continuation>::parse(context, input, sink,
                                                              LEXY_FWD(args)...);
        }
        else
        {
